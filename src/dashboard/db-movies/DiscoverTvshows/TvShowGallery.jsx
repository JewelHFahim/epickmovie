...
const TvShowGallery = ({ tvShows }) => {
  ...
  const [importSingleTvshow] = useImportSingleTvshowMutation();
  ...
  const handleImageClick = async (id) => {
    ...
    // const res = importSingleTvshow(data);
    // console.log(res)
    // try {
    //   console.log(res)
    //   if (res && res.data) {
          ...
          {notificationList?.map((item) => (
            ...
            >
              <div className="flex items-center gap-x-8 text-sm">
                <p className="  font-medium text-green-600 uppercase">
                  Imported
                </p>
                ...
                <p className="text-blue-700 ml-10">
        ...
        {tvShows?.map((image) => (
          <div
            ...
            className={`relative cursor-pointer ${
              uploadedIds?.data?.includes(image.id.toString())
                ? "cursor-not-allowed"
                : ""
            } m-[8px]
             ${
          ...
          >
            ...
            {uploadedIds?.data?.includes(image.id.toString()) && (
              <div className="absolute w-full h-full bg-black bg-opacity-[40%] top-0 left-0">
                <p className="text-white flex justify-end text-2xl p-4">
