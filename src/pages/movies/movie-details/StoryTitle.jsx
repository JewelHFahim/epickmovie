const StoryTitle = ({ details }) => {
  return (
    <div className="mt-[11px] lg:mt-[48px] lg:max-w-[745px] text-white font-roboto flex flex-wrap">
      <p className="text-[30px] lg:text-[15px]">
        <span className="font-bold">{details?.post_title}: </span>
        This is one of the best movie based on
        {details?.additional_data?.genres?.map((item, i) => (
          <span key={i}>
            <span className="text-red-600 font-semibold mx-1">{item.term.name}</span>
            ,
          </span>
        ))}
        ...
        . This Movie is Available in Hindi with English Subtitles. This Movie Is
        Now Available In
        {details?.additional_data?.dtaudio?.map((item, i) => (
          <span key={i}>
            <span className="text-[#ff007a] font-semibold ml-1">
              {item.term.name}
            </span>
          </span>
        ))}
